...
func seedNovelReportDB(t *testing.T) novelSeed {
	...
	insertOrder := func(id, name string, created time.Time, status, source, customerID, email string, total, refund float64, discounted bool, tags []string, items string) {
		t.Helper()
		discounts := "[]"
		if discounted {
			discounts = `[{"__typename":"DiscountCodeApplication","targetType":"LINE_ITEM"}]`
		}
		data := fmt.Sprintf(`{"id":%q,"name":%q,"totalPriceSet":{"shopMoney":{"amount":"%.2f","currencyCode":"USD"}},"totalRefundedSet":{"shopMoney":{"amount":"%.2f","currencyCode":"USD"}},"customer":{"id":%q,"email":%q},"tags":%s,"discountApplications":{"nodes":%s},"lineItems":{"nodes":%s}}`, id, name, total, refund, customerID, email, mustMarshalString(t, tags), discounts, items)
	...
	insertOrder("o-old-c1", "#1000", now.AddDate(0, 0, -400), "PAID", "web", "gid://shopify/Customer/1", "c1@example.com", 70, 0, false, nil, `[{"id":"li-old","title":"Legacy A","quantity":1,"originalUnitPriceSet":{"shopMoney":{"amount":"70.00"}},"variant":{"id":"v-old","sku":"SKU-A","product":{"id":"pA","title":"Widget A","handle":"widget-a"}}}]`)
	insertOrder("o-cur-a", "#1001", now.AddDate(0, 0, -2), "PAID", "web", "gid://shopify/Customer/1", "c1@example.com", 100, 0, true, []string{"Klaviyo", "VIP"}, `[{"id":"li-1","title":"Widget A","quantity":2,"originalUnitPriceSet":{"shopMoney":{"amount":"20.00"}},"variant":{"id":"vA","sku":"SKU-A","product":{"id":"pA","title":"Widget A","handle":"widget-a"}}},{"id":"li-2","title":"Widget B","quantity":1,"originalUnitPriceSet":{"shopMoney":{"amount":"60.00"}},"variant":{"id":"vB","sku":"SKU-B","product":{"id":"pB","title":"Widget B","handle":"widget-b"}}}]`)
	insertOrder("o-cur-refund", "#1002", now.AddDate(0, 0, -1), "PARTIALLY_REFUNDED", "web", "gid://shopify/Customer/1", "c1@example.com", 50, 10, false, nil, `[{"id":"li-3","title":"Widget A","quantity":1,"originalUnitPriceSet":{"shopMoney":{"amount":"50.00"}},"variant":{"id":"vA","sku":"SKU-A","product":{"id":"pA","title":"Widget A","handle":"widget-a"}}}]`)
	insertOrder("o-prev", "#1003", now.AddDate(0, 0, -10), "PAID", "pos", "gid://shopify/Customer/2", "c2@example.com", 80, 0, false, nil, `[{"id":"li-4","title":"Widget B","quantity":2,"originalUnitPriceSet":{"shopMoney":{"amount":"40.00"}},"variant":{"id":"vB","sku":"SKU-B","product":{"id":"pB","title":"Widget B","handle":"widget-b"}}}]`)
	insertOrder("o-web-c2", "#1004", now.AddDate(0, 0, -20), "PAID", "pos", "gid://shopify/Customer/2", "c2@example.com", 200, 0, false, nil, `[{"id":"li-5","title":"Widget B","quantity":2,"originalUnitPriceSet":{"shopMoney":{"amount":"100.00"}},"variant":{"id":"vB","sku":"SKU-B","product":{"id":"pB","title":"Widget B","handle":"widget-b"}}}]`)
	insertOrder("o-email-c3", "#1005", now.AddDate(0, 0, -40), "PAID", "email", "gid://shopify/Customer/3", "c3@example.com", 30, 0, false, []string{"newsletter"}, `[{"id":"li-6","title":"Widget C","quantity":1,"originalUnitPriceSet":{"shopMoney":{"amount":"30.00"}},"variant":{"id":"vC","sku":"SKU-C","product":{"id":"pC","title":"Widget C","handle":"widget-c"}}}]`)
...
func runNovelReport(t *testing.T, dbPath string, args ...string) any {
	...
	cmd.SetOut(&out)
	cmd.SetErr(&errOut)
	full := append([]string{"--json", "report", "--db", dbPath}, args...)
	cmd.SetArgs(full)
